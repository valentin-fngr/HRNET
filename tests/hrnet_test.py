...
import numpy as np 
import torch 
from model.hrnet_v2 import FusionModule, HRNETV2, Hrnet_v2_head, HeatMapRegressionLoss
...
class TestHRNETModules(unittest.TestCase):
    ...
    def test_heatmap_regression_loss(self): 
        """
        Test the computation of heatmap regression
        """
        bs = 10 
        nb_joints = 17 
        c_out = 64
        true = torch.randn(bs, nb_joints, c_out)
        pred = torch.randn(bs, nb_joints, c_out)

        cirterion = HeatMapRegressionLoss()

        loss = cirterion(pred, true)
